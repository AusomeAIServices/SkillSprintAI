...
The home preview begins with **What is AI?**, a self-paced sequence of three waves, nine milestones, and 27 bite-sized topics. Each milestone has an Easy, Hard, and Advanced topic. The first topic is **What is AI?**, a plain-language introduction to AI and ChatGPT; the second is **From Google search to ChatGPT**. **Ask AI to explain it your way** follows in the Learn with AI milestone. The learner reads a plain-language explanation and a small practice, then chooses **AI Understand** to advance. This is a self-attested navigation choice, not a scored knowledge check. The optional F01 button opens the complete 15-minute draft lesson. A separate prompt helper supports Simple words, “Explain like I’m 5,” or Tagalog and copies a prompt for use in ChatGPT; this preview does not make a model call.

### What is AI? journey waves and milestones

1. **Get comfortable with AI:** Start a useful conversation (What is AI? → Google vs ChatGPT → use both together); Make your request clear (say what you want → add useful context and limits → specify and refine the output); Learn with AI (ask AI to explain it your way → one-question-at-a-time practice → teach it back).
...
Each of the 27 topics has three authored, fictional prompt-and-response examples in an offline chat simulator (81 total). The learner selects a scenario, can inspect or edit its prompt, and reveals its scripted reply only for the unedited sample prompt. Edited prompts receive an explicit notice that the preview cannot generate a new answer; no subscription, model call, or external chat session is involved. These examples demonstrate the topic and do not act as assessment keys.
