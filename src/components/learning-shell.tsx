...
export default function LearningShell() {
  ...
  return <><TopBar /><div className="app-layout" id="today">
    ...
    <main className="main-column">
      <div className="welcome-row"><div><div className="eyebrow">YOUR DAILY PRACTICE</div><h1>New to AI? Start with what you already know.</h1><p>Start with a simple explanation of AI, then see how ChatGPT differs from Google Search. No AI experience needed.</p></div>
  ...
  return <main className="lesson-wrap">
    ...
    <section className="lesson-panel" aria-labelledby="step-title"><header className="lesson-panel-head"><div className="eyebrow">STEP {index + 1} · ABOUT {step.minutes} MIN</div><h2 id="step-title" tabIndex={-1}>{step.title}</h2><p>{stepDescriptions[props.currentStep]}</p></header>
      <div className="lesson-panel-body">
        ...
        {props.currentStep === "learn" && <><p className="lesson-copy">{step.text}</p><div className="example-card weak"><span className="example-label">A broad request</span><p>{step.weakExample}</p></div><div className="example-card"><span className="example-label">A more useful request</span><p>{step.workedExample}</p></div><div className="callout">{step.privacyNote}</div><section className="prompt-breakdown" aria-labelledby="prompt-parts-title"><h3 id="prompt-parts-title">The four parts, in this example</h3><p>Context is the useful background you give for this request. In a conversation, relevant earlier messages can also provide context. ChatGPT uses context to tailor its reply, but it does not automatically verify those details.</p><dl className="prompt-parts"><div className="prompt-part"><dt>Goal</dt><dd>What do you want ChatGPT to do?<br /><span>Plan a quiet afternoon.</span></dd></div><div className="prompt-part"><dt>Context</dt><dd>What useful background should it know?<br /><span>One adult, 1–4 pm, starting at the library.</span></dd></div><div className="prompt-part"><dt>Constraints</dt><dd>What limits or preferences should it follow?<br /><span>Spend no more than $12; leave 30 minutes free; use only the supplied facts.</span></dd></div><div className="prompt-part"><dt>Output</dt><dd>How should the answer be presented?<br /><span>A simple schedule table, plus anything the facts do not tell us.</span></dd></div></dl><p className="context-note">Quick memory aid: <strong>Goal = what to do. Context = what it needs to know. Constraints = limits to respect. Output = what the answer should look like.</strong></p></section></>}
